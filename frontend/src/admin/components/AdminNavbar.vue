...
const handleSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    ...
    try {
      ...
      ;(lic || []).forEach(l => {
        results.push({ id: 'l-' + l.id, label: l.referencia || 'Sem referência', meta: (l.cliente_nome || '') + (l.empresa ? ' · ' + l.empresa : ''), icon: 'bi bi-file-earmark-check', link: `/admin/licenciamentos/${l.id}` })
      })
