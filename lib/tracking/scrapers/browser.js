...
export function detectCarrier(value) {
  ...
  if (/^LISLAD|^PSL/.test(v)) return { carrier: 'naiber', name: 'Naiber / PSL' };

  // BL number detection (non-container formats)
  if (/^S\d{9}$/.test(v)) return { carrier: 'grimaldi', name: 'Grimaldi' };
  if (/^\d{10,12}$/.test(v)) return { carrier: 'orey', name: 'OREY' };

  // CMA CGM BL patterns: 3 letters + 7 digits (GGZ2993831, XIA1868409, etc.)
  if (/^[A-Z]{3}\d{7}$/.test(v) && !/^HLC|^MSD|^MSK|^MSC|^MED|^CMA/.test(v)) {
    return { carrier: 'cmacgm', name: 'CMA CGM' };
  }

  // Maersk BL: 9 digits starting with 2
  if (/^2\d{8}$/.test(v)) return { carrier: 'maersk', name: 'Maersk' };
