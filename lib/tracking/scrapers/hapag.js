...
export async function trackHapag(value) {
  ...
  try {
    ...
    await page.waitForSelector('input[name="tracking-number"], input[id*="tracking"]', { timeout: 10000 });
    ...
    const events = await page.evaluate(() => {
      const rows = document.querySelectorAll('table tbody tr, .tracking-event, .event-row');
      const results = [];
      rows.forEach(row => {
